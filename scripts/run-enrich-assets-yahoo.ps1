...
$uri = "$base/functions/v1/enrich-assets-yahoo"
...
Write-Host "Calling enrich-assets-yahoo..." -ForegroundColor Cyan
Write-Host "- URL: $uri" -ForegroundColor Cyan
Write-Host "- Key file: $keyPath" -ForegroundColor Cyan
if ($bodyObj.ContainsKey('symbols') -and $bodyObj['symbols'] -and $bodyObj['symbols'].Count -gt 0) {
  Write-Host "- symbols=$($bodyObj['symbols'] -join ',')" -ForegroundColor Cyan
}

$resp = Invoke-RestMethod -Method Post -Uri $uri -Headers $headers -Body $body -TimeoutSec $TimeoutSec
"enrich-assets-yahoo response:"
